...
export class IQWSClient extends EventEmitter {
  ...
  private handleMessage(message: any): void {
    console.log(`[WS] Mensagem recebida: ${message.name} (request_id: ${message.request_id})`);
    ...
    if (message.request_id && this.pendingRequests.has(message.request_id)) {
      ...
      if (message.name === 'result' || message.msg) {
        ...
        pending.reject(new Error(`Erro na resposta: ${JSON.stringify(message)}`));
      }
    }
  ...
  async send(name: string, version: string, body: any, requestId?: string): Promise<any> {
  ...
  private sendRaw(message: any): void {
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      console.log(`[WS] Enviando: ${message.name} (request_id: ${message.request_id})`);
  ...
  async getInstruments(type: string): Promise<MarketLite[]> {
    ...
    try {
      ...
      console.log(`[WS] Encontrados ${instruments.length} instruments para ${type}`);
      return instruments;
    } catch (error) {
  ...
  async getInitializationData(): Promise<{ binary: MarketLite[]; turbo: MarketLite[] }> {
    ...
    try {
        ...
        if (data.binary?.actives) {
          const binaryActives = Object.entries(data.binary.actives)
            .map(([key, obj]: [string, any]) => {
              ...
              const name = obj?.name || obj?.asset || obj?.ticker || obj?.symbol || String(active_id);
              const id = active_id;
              return {
                id,
                name,
      ...
      console.log(`[BINARY] Dados obtidos: ${result.binary.length} binary, ${result.turbo.length} turbo`);
      return result;
    } catch (error) {
  ...
  private mapInstrumentType(type: string): 'crypto' | 'forex' | 'cfd' | 'digital' {
    switch (type.toLowerCase()) {
      case 'crypto': return 'crypto';
      case 'forex': return 'forex';
      case 'cfd': return 'cfd';
      case 'digital-option': return 'digital';
