...
// Mock do IQWSClient
jest.mock("../../src/services/iq/ws-client", () => ({
  IQWSClient: jest.fn().mockImplementation(() => ({
...
// Mock do IQOrderHelper
jest.mock("../../src/helpers/iqOrderHelper", () => ({
  IQOrderHelper: {
    calculateExpirationTimestamp: jest
      .fn()
      .mockReturnValue(Math.floor(Date.now() / 1000) + 120),
    buildInstrumentId: jest.fn().mockReturnValue("do76A20250830D150000T1MCSPT"),
    calculateValue: jest.fn().mockReturnValue(1234567890),
  },
}));
...
describe("Order Service Tests", () => {
  ...
  describe("createAutomaticOrder", () => {
    ...
    test("deve lançar erro para estratégia inexistente", async () => {
      ...
      await expect(
        OrderService.createAutomaticOrder(orderData)
  ...
  describe("processOrderResult", () => {
    test("deve processar ordem como WIN e calcular lucro", async () => {
      ...
      await OrderService.processOrderResult(order.id, "win", 1.086);
      ...
      expect(updatedOrder?.closePrice).toBe(1.086);
    });
    ...
    test("deve processar ordem como LOSS e calcular prejuízo", async () => {
      ...
      await OrderService.processOrderResult(order.id, "loss", 1.086);
    ...
    test("deve lançar erro para ordem inexistente", async () => {
      await expect(
        OrderService.processOrderResult(99999, "win")
      ).rejects.toThrow(AppError);
    });
  });
  ...
  describe("sendOrderToIQOption", () => {
    test("deve criar ordem manual para envio à IQ", async () => {
      // Mock da resposta da IQ Option
      const mockIQWSClient =
        require("../../src/services/iq/ws-client").IQWSClient;
      const mockClient = {
        connect: jest.fn().mockResolvedValue(true),
        isConnected: jest.fn().mockReturnValue(true),
        send: jest.fn(),
        on: jest.fn((event, callback) => {
          // Simular resposta imediata da IQ
          if (event === "message") {
            setTimeout(() => {
              callback({
                name: "option-opened",
      ...
      mockIQWSClient.mockImplementationOnce(() => mockClient);
      ...
      expect(order).toBeDefined();
      expect(order.asset).toBe("EURUSD-OTC");
      expect(order.type).toBe("call");
      expect(order.amount).toBe(10);
      expect(order.source).toBe("manual");
      expect(order.strategyId).toBeNull();
